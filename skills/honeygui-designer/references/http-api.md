...
## ⚠️ 调用注意事项（Windows / 非交互环境，实测踩坑）

本文档示例用 `curl`，但在 Windows + 非交互（NonInteractive）shell 下有几个会**整批失败**的坑：

1. **PowerShell 里 `curl` 是 `Invoke-WebRequest` 的别名**，不是真 curl。
   - 首选改调真 curl：写 **`curl.exe`**（Windows 10+ 自带），绕开别名，跨平台行为一致。
   - 若坚持用 PowerShell 原生 cmdlet：用 **`Invoke-RestMethod`**（自动解析 JSON）并加 **`-UseBasicParsing`**，
     否则非交互模式会因缺少 IE 引擎直接报错，且报错像"网络问题"，极不直观。
2. **不要用 `Start-Job` / `Wait-Job` 等并行/后台方式发请求**——非交互下无法弹 prompt，会整批挂死（死锁）。
   **顺序调用**即可；本地服务很快，无需并行。
3. **含 `"` 的 body（如 SVG）——按环境选写法**：

   **默认 / bash（Linux、macOS、Windows 的 git bash）** — 单引号包裹直接内联即可。SVG 用双引号属性、不含单引号，
   bash 单引号会原样保住整段（已实测 `JSON.parse` 通过）：
   ...
   **body 特别复杂时（任意平台，最稳）** — 写临时文件再 `curl --data-binary @file`，绕开所有 shell 转义。

   **仅 PowerShell 非交互环境** — PS 引号规则 + 调原生 exe 会再解析一遍 + `curl` 是 `Invoke-WebRequest` 别名，
   三者叠加会拆坏内联 JSON，必须走"文件 + `--data-binary`"。PS 专属注意：`curl.exe`（非别名 `curl`）、
   `python`（Windows 没 `python3`）、`Remove-Item`（非 `rm`），且**别用 `Out-File -Encoding utf8`**
   （PS 5.1 会写 UTF-8 BOM，API 不接受）：

   ```powershell
   $svg = (curl.exe -s "https://api.iconify.design/mdi/bluetooth.svg?color=%232563EB&width=96")
   python -c "import json,sys; open('_req.json','w',encoding='utf-8').write(json.dumps({'svg':sys.argv[1],'name':'icon_bt','width':96}))" "$svg"
   curl.exe -X POST http://localhost:38912/api/svg-to-image -H "Content-Type: application/json" --data-binary @_req.json
   Remove-Item _req.json
   ```

```powershell
# PowerShell 原生写法（等价于文档里的 curl 示例）
Invoke-RestMethod -Uri http://localhost:38912/api/validate-hml -Method Post `
  -ContentType 'application/json' -UseBasicParsing `
  -Body '{"filePath":"ui/main.hml"}'
```
...
### 7. POST /api/svg-to-image

把 SVG 字符串栅格化为 PNG，写入项目 `assets/`，返回供 HML 引用的路径。
图标取材完整流程见 `references/icon-sourcing.md`：从 Iconify 检索现成矢量图 → 清洗 → 交本端点栅格化。

**请求**：
```bash
curl.exe -X POST http://localhost:38912/api/svg-to-image \
  -H "Content-Type: application/json" \
  -d '{"svg":"<svg viewBox=\"0 0 24 24\" xmlns=\"http://www.w3.org/2000/svg\"><path fill=\"#2563EB\" d=\"...\"/></svg>","name":"icon_bt","width":96}'
```
...
| 参数 | 类型 | 必需 | 说明 |
|------|------|------|------|
| `svg` | string | 是 | SVG 字符串（已清洗：颜色确定、无外部/网络引用）|
| `name` | string | 是 | 资源名，仅 `[a-zA-Z0-9_-]`，不含扩展名与路径 |
| `width` | number | 是 | 输出 PNG 像素宽度 |
| `height` | number | 否 | 省略则按 SVG viewBox 比例 |
| `overwrite` | boolean | 否 | 默认 `false`；同名已存在返回 409 |
...
```json
{
  "success": true,
  "data": {
    "assetPath": "assets/icon_bt.png",
    "absolutePath": "...",
    "width": 96,
    "height": 96,
    "bytes": 4400,
    "loadedFonts": ["NotoSansSC-Medium"]
  }
}
```

- 产物到 PNG 即止；PNG → `.bin` 由仿真/构建期自动处理。
- `loadedFonts` 为兼容字段（列出自动注入的 `assets/*.ttf`）；图标取材用矢量图标、不涉及 `<text>`，通常可忽略。
...
| `POST /api/svg-to-image` | SVG 栅格化为 PNG 写入 assets/ | 否（需打开项目）|
...
| `ASSET_EXISTS` | 409 | 同名资源已存在（`svg-to-image` 默认防覆盖）；加 `"overwrite":true` 可强制替换 |
